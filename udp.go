...
func timedCopy(dst net.PacketConn, target net.Addr, src net.PacketConn, timeout time.Duration, role mode) error {
	...
	for {
		if err := src.SetReadDeadline(time.Now().Add(timeout)); err != nil {
			logf("timedCopy err:%s", err.Error())
			return err
		}
		n, raddr, err := src.ReadFrom(buf)
		if err != nil {
			logf("readfrom err:%s", err.Error())
			continue
		}
		...
		case remoteServer: // server -> client: add original packet source
			...
			logf("remoteServer to:%s", target.String())
		case relayClient: // client -> user: strip original packet source
			srcAddr := socks.SplitAddr(buf[:n])
			_, err = dst.WriteTo(buf[len(srcAddr):n], target)
			logf("relayClient to:%s", target.String())
		case socksClient: // client -> socks5 program: just set RSV and FRAG = 0
			_, err = dst.WriteTo(append([]byte{0, 0, 0}, buf[:n]...), target)
			logf("socksClient to:%s", target.String())
		}

		if err != nil {
			logf("timecopy err:%s", err.Error())
			continue
		}
	}
}
