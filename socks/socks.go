...
func Handshake(rw io.ReadWriter, authaddr string) (Addr, error) {
	...
	ok := auth.Auth(authaddr, user.Uname)
	if !ok {
		if _, err := rw.Write([]byte{1, 1}); err != nil {
			return nil, err
		}
...
func authForm(r io.ReadWriter) (user User, err error) {
	bb := make([]byte, 2)
	if _, err := io.ReadFull(r, bb); err != nil {
		return user, errors.New("parse auth err:read io")
	}
	if bb[0] != 0x01 {
		return user, errors.New("parse auth err:bad socks5 version")
	}
	if bb[1] == 0 {
		return user, errors.New("parse auth err:bad user lenth")
	}
	ub := make([]byte, int(bb[1])+1)
	if _, err := io.ReadFull(r, ub); err != nil {
		return user, errors.New("parse auth err:read user text err")
	}
	if ub[int(bb[1])] == 0 {
		return user, errors.New("parse auth err:bad user text err")
	}
	p := make([]byte, int(ub[int(bb[1])]))
	if _, err := io.ReadFull(r, p); err != nil {
		return user, errors.New("parse auth err:read password")
	}
	// fmt.Printf("Got UserPassNegotiationRequest: %#v %#v %#v %#v %#v\n", bb[0], bb[1], ub[:int(bb[1])], ub[int(bb[1])], p)
